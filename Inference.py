...
def inference(pose_model, save_path, video_path, best_model='best_model.pt', device=DEVICE):
    yolo = YOLO(pose_model)
    device = torch.device(device)

    detect_model = LSTM(INPUT_DIM, HIDDEN_DIM, NUM_LAYERS, OUTPUT_DIM).to(device)
    model_weight = torch.load(os.path.join(save_path, best_model))
    detect_model.load_state_dict(model_weight)
    with open(os.path.join(save_path, 'idx_2_category.json'), 'r') as f:
